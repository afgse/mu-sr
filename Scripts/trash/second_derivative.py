...
x_units = 'mm'
y_name = 'B'
y_units = 'Gauss/mm^2'  # second deriv
...
for R in np.arange(vert):
   for i in np.arange(horiz):
       ...
       def biotSecond(z,R,sep):
           indep = (z+tau/2)-sep
           return constant * ((15 * R**2 * indep**2 / (indep**2 + R**2)**(7/2))
                              - 3*R**2 / (indep**2 + R**2)**(5/2))

       y1 += biotSecond(z,tau/2+R*met,i*met)
...
for R in np.arange(vert):
   for i in np.arange(horiz):
       ...
       y2 += biotSecond(z,tau/2+R*met,i*met)
